...
\usepackage{common}
\usepackage{booktabs}
\title{Practical 1: Draft}
\author{Antonio Copete \\ acopete@cfa.harvard.edu \\
...
\item 256 predictors. Linear Regression $R^2 = 0.46$, Random Forest $R^2 = 0.55$.
\item 194 features are not significant in LR (coef = 0.0), and 225 in RF (importance = 0.0). All 194 unimportant LR predictors are also unimportant in RF. Drop them.
\item Testing with only the 31 features that have RF importance $> 0.0$ gives the same $R^2$ in both cases, up to 5 significant digits. Keep working with these features only. Also, no molecules in the training set have ANY of the unimportant 225 features. Definitely drop them!


\end{itemize}


\section*{Template begins here:}
